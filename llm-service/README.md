...
Esta carpeta es autocontenida: **todo el TP hasta la fecha vive acá adentro** (backend, frontend
de prueba y documentación). Se puede copiar/mover `llm-service/` entera a otro lado y todos los
...
|---|---|---|
| **[`docs/importado/`](docs/importado/)** | `Brf93` (`421562`) | Anidado adentro de `docs/`, para que la documentación tenga una sola carpeta. 25 documentos: especificación técnica, nueve planes de ejecución e investigación sobre jailbreak. Material de profundización, no de trabajo diario |
| **[`llm-workbench/`](llm-workbench/)** | el equipo | Frontend docente en Angular 21 (evaluador de uso de IA, rúbricas, golden set y calibración). Se levanta con Docker Compose (`compose.workbench.yaml`) o localmente vía Angular CLI |
...
| **[`docs/fuentes/`](docs/fuentes/)** | cátedra | PDFs oficiales del PRD y propuesta de arquitectura del TPI (`*.pdf`, gitignored globalmente — viajan con la carpeta si se copia a mano, no si se clona el repo) |
...
- [`docs/fuentes/PRD-Plataforma-Gamificada-TP.pdf`](docs/fuentes/) (v2.1) — definición funcional del producto
- `idea.pptx.pdf` — convenciones obligatorias de Gateway, Eureka, seguridad, ruteo y pruebas (canales de la cátedra, no versionado acá)
- [`docs/fuentes/TUP_PIV_BE_PROPUESTA_ARQ.pdf`](docs/fuentes/) — propuesta de arquitectura de la cátedra
- [`docs/fuentes/TUP_PIV_FE_TEO_U1_ARQUITECTURA_DESPLIEGUE.pdf`](docs/fuentes/) — teórico de Front End, Unidad 1: arquitectura y
  despliegue. Sincronizado en [15](docs/15-sincronizacion-arquitectura-y-despliegue.md)

> Los PDF están en `docs/fuentes/` pero gitignorados (`*.pdf`): si se clona el repo no vienen, pero
> si se copia esta carpeta a mano sí viajan con ella. Se distribuyen también por los canales de la cátedra.

## Cómo se armó esta documentación
...
*Documentación viva. Última actualización: 2026-09-12 (consolidación de `docs/` y `llm-workbench/`
dentro de `llm-service/`, ver [37 §0.1](docs/37-estructura-carpetas-backend.md#01-por-qué-docs-y-llm-workbench-viven-ahora-dentro-de-llm-service)).*
