...
@Repository
public class ChallengeCalibrationAssignmentRepository {
  ...
  public boolean lockOnFirstAttempt(UUID challenge,UUID attempt){return j.update("update llm.challenge_calibration_assignments set first_attempt_id=?,locked_at=now() where challenge_id=? and first_attempt_id is null",attempt,challenge)==1;}
  private record Row(UUID id,UUID run,java.time.OffsetDateTime lockedAt){}
}
