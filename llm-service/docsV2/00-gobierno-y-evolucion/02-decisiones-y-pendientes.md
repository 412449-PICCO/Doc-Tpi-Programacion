...
> **Actualización normativa.** Los ADR y pendientes se leen bajo la precedencia de [00](01-fuentes-de-verdad-y-convenciones.md). Quedan cerrados para planificación: `llm-service`, `/api/llm/**`, Kafka como bus compartido y API por recursos; los ADR anteriores que usen otro nombre, dispatcher `/ai/{funcion}` o `trace_id` son históricos.
...
📄 [02](../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md)
...
📄 [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md)
...
📄 [02](../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md)md)
...
📄 [04](../03-capacidades-de-ia/02-funciones-de-ia.md)
...
📄 [05](../04-seguridad-datos-y-cumplimiento/01-seguridad-y-guardarrailes.md)
...
> ([14](../08-preguntas-investigacion-y-sincronizaciones/02-sincronizacion-con-guia-didactica.md) C-2): se transmite, pero la entrega al alumno se retiene
...
del texto crudo — la hipótesis está desarrollada en [03](../03-capacidades-de-ia/01-modelos-costos-y-contexto.md) §1, y de
confirmarse este escenario hay que rehacerlo con el número medido.

📄 [03](../03-capacidades-de-ia/01-modelos-costos-y-contexto.md)
...
1. **Latencia** — el presupuesto es < 300 ms ([02](../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md)) y un roundtrip HTTP
...
2. **Es la red del fail-open** — sin capa clásica, el fail-open de P-02 significa *sin ninguna
   moderación*.
3. **Datos** — cuantos menos mensajes de alumnos salgan del sistema, mejor ([07](../04-seguridad-datos-y-cumplimiento/02-datos-retencion-y-terminos.md)
4. **Elimina el prompt del moderador** — y con él la superficie de prompt injection sobre el
   ...
   [14](../08-preguntas-investigacion-y-sincronizaciones/02-sincronizacion-con-guia-didactica.md)) y el pendiente E-05. Un clasificador no interpreta
...
en [03](../03-capacidades-de-ia/01-modelos-costos-y-contexto.md) es USD 0 porque no hay nada que facturar, no porque se haya
estimado con optimismo. La tabla está en [04](../03-capacidades-de-ia/02-funciones-de-ia.md) §2.0.
...
seis pedidos habituales que **no** lo justifican— están enumerados en [04](../03-capacidades-de-ia/02-funciones-de-ia.md)
...
[04](../03-capacidades-de-ia/02-funciones-de-ia.md) §2.3.4b y §2.3.4c. En resumen: **Pipes and Filters** para la
...
**Idempotent Receiver**— ya venía de ADR-001 y de [02](../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md)
...
📄 [03](../03-capacidades-de-ia/01-modelos-costos-y-contexto.md)egia de despliegue
...
regla de versionado del contrato de [02](../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md) —*"un cambio incompatible sin
...
| --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Blue-Green**  | Duplica la infraestructura, y su punto débil declarado son las migraciones de base. Las nuestras son append-only ([07](../04-seguridad-datos-y-cumplimiento/02-datos-retencion-y-terminos.md)§3.3), y van camino a triggers de inmutabilidad en la base ([14](../08-preguntas-investigacion-y-sincronizaciones/02-sincronizacion-con-guia-didactica.md) A-5): dos esquemas vivos a la vez es peor acá que en un CRUD |
...
[15](../02-arquitectura-y-plataforma/02-sincronizacion-arquitectura-y-despliegue.md)
...
📄 [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md)md)
...
**Por qué:** la [U1 de Front End](../02-arquitectura-y-plataforma/02-sincronizacion-arquitectura-y-despliegue.md) enseña a Nginx como
gateway de microservicios, con un `location` por servicio. Aplicado acá chocaría de frente con dos
reglas no negociables de [02](../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md): *"el API Gateway es la única puerta de
...
[02](../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md) marcaba "SDK oficial · `RestClient` directo" como 🟡 *menos
...
📄 [00](01-fuentes-de-verdad-y-convenciones.md)bración
...
📄 [00](01-fuentes-de-verdad-y-convenciones.md) antes de reabrir una discusión.** Nueve decisiones cambiaron mientras se armaba la
...
| 4 | Sin streaming en prácticos | **Buffer Interceptor** | La guía didáctica lo resuelve mejor ([14](../08-preguntas-investigacion-y-sincronizaciones/02-sincronizacion-con-guia-didactica.md) C-2) |
...
> [14](../08-preguntas-investigacion-y-sincronizaciones/02-sincronizacion-con-guia-didactica.md)
...
📄 [00](01-fuentes-de-verdad-y-convenciones.md)md)
...
📄 [03](../03-capacidades-de-ia/01-modelos-costos-y-contexto.md) §4b
...
📄 [04](../03-capacidades-de-ia/02-funciones-de-ia.md) Parte 4
...
([`contracts/equipos/tema-12-backoffice-admin.md`](../contracts/equipos/tema-12-backoffice-admin.md)
...
pantalla que llama a ese endpoint extendido. Ver [`ep-07`](../07-planificacion-y-trabajo-equipo/09-epicas-historias-tareas-sprints/epicas/ep-07.md)
[`historias/ep-07/h02.md`](../07-planificacion-y-trabajo-equipo/09-epicas-historias-tareas-sprints/historias/ep-07/h02.md)
...
📄 [ep-07](../07-planificacion-y-trabajo-equipo/09-epicas-historias-tareas-sprints/epicas/ep-07.md)· [`contracts/equipos/tema-12-backoffice-admin.md`](../contracts/equipos/tema-12-backoffice-admin.md)
...
- **Purga selectiva del chat social.** RF-CHT-08 borra el canal al archivar el curso, pero RF-CHT-14
  ...
  borrado por curso. Ver [04](../03-capacidades-de-ia/02-funciones-de-ia.md) Parte 4.
...
> ADR-012 a ADR-015, el documento [15](../02-arquitectura-y-plataforma/02-sincronizacion-arquitectura-y-despliegue.md) completo y el
...
> Boot, que [10](../01-vision-alcance-y-entrega/02-entregables-y-plan.md) §7 todavía lista como pregunta abierta de la semana.
...
| — | Tamaño del equipo | 🟢 **12 integrantes** (5 parejas P1–P5 + referente de producto + facilitador). El "6 personas / P1–P6" de [10](../01-vision-alcance-y-entrega/02-entregables-y-plan.md)quedó superado; reparto vigente en [23 · §3](../07-planificacion-y-trabajo-equipo/03-plan-de-construccion-del-producto.md)
| — | ¿Producto o demo? | 🟢 **Tiene que funcionar, pero primero demo local.** El "plan de 4 semanas" de [10](../01-vision-alcance-y-entrega/02-entregables-y-plan.md)fue reemplazado por los 19 sprints de [23](../07-planificacion-y-trabajo-equipo/03-plan-de-construccion-del-producto.md), y ese horizonte a su vez por el máximo de **5 sprints** de [38](../07-planificacion-y-trabajo-equipo/05-plan-de-cinco-sprints.md)
...
**Ojo:** la metadata de tiempos entre mensajes y ediciones de código ([07](../04-seguridad-datos-y-cumplimiento/02-datos-retencion-y-terminos.md) §3.1) es **evidencia de la dimensión que más pesa** y no se puede reconstruir después. Si la guarda otro equipo, **hay que pedirles explícitamente que la capturen** — no la van a capturar solos.
...
> [`contracts/equipos/tema-05-desafios-practicos.md`](../contracts/equipos/tema-05-desafios-practicos.md#qué-nos-dan-el-cierre-del-intento)
...
> [`equipos/tema-11-chat/pendientes.md`](../07-planificacion-y-trabajo-equipo/11-equipos/tema-11-chat/pendientes.md#-cruzado--b-3-los-campos-que-tema-11-tiene-que-incluir-en-el-contrato-de-eventos)
...
Conceptualmente ya está resuelto ([01](../01-vision-alcance-y-entrega/01-problema-y-alcance.md) §2c) y la cátedra coincide. **Falta escribirlo en el contrato.**
...
[`estado-implementacion/ep-06/evaluacion-y-apelacion.md`](../06-operacion-calidad-y-pruebas/04-estado-de-implementacion/ep-06/evaluacion-y-apelacion.md)
...
| Fuente | Dice |
|---|---|
| [`sprints/README.md` · reprogramación a 8 semanas](../07-planificacion-y-trabajo-equipo/09-epicas-historias-tareas-sprints/sprints/README.md) | Alcance estricto = 6 ítems (rúbrica, golden set, invocación de modelo, calibración, bloqueo de activación, salvaguarda anti-fuga). **EP-06 no está en la lista.** Se autodeclara *"nota de planificación, no fuente de verdad"* |
| [`00-fuentes-de-verdad-y-convenciones.md` §5](01-fuentes-de-verdad-y-convenciones.md) | El **MVP** incluye *"evaluador asíncrono... apelación, score diferido y bloqueo de cierre"* — y este documento **sí** es fuente de verdad, con más jerarquía que la nota de arriba |

**Consecuencia de cada opción:**

| Si la respuesta es... | Pasa esto |
|---|---|
| **Sí, es parte de esta entrega** | Faltan **~2 sprints más** (S6 + S7 de [`35-backlog-ejecutable.md`](../07-planificacion-y-trabajo-equipo/04-backlog-ejecutable.md), ~416 h) que hoy están en 0% — hay que reservarles pareja y calendario ya |
...
📄 [`estado-implementacion/ep-06/evaluacion-y-apelacion.md`](../06-operacion-calidad-y-pruebas/04-estado-de-implementacion/ep-06/evaluacion-y-apelacion.md)
[00](01-fuentes-de-verdad-y-convenciones.md)po/09-epicas-historias-tareas-sprints/sprints/README.md)
...
|---|---|---|---|
| 🟢 D-1 | Lenguaje del servicio | **Java Spring Boot**, igual que el resto de la plataforma. Ver [02](../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md) | Hagan falta otros lenguajes en los desafíos, o embeddings locales |
...
Para que no se vuelva a discutir. Detalle en [08](02-decisiones-y-pendientes.md)d)
...
|---|---|---|---|---|---|
| E-01 | **Las anclas de las 5 dimensiones** | [13](../03-capacidades-de-ia/03-rubricas-y-prompts.md) §3 | 📝 | **Docente del curso**, ajustando sobre el borrador | Antes de publicar la rúbrica |
| E-02 | Prompt del evaluador | [13](../03-capacidades-de-ia/03-rubricas-y-prompts.md) §6 | 📝 | P3 | Paso 5 |
| E-03 | Prompt del tutor + las 3 variantes de riesgo | [13](../03-capacidades-de-ia/03-rubricas-y-prompts.md) §7 | 📝 | P5 + P6 | Paso 11 |
| E-04 | Prompt del generador | [13](../03-capacidades-de-ia/03-rubricas-y-prompts.md) §8 | 📝 | P2 | Paso 10 |
| ~~E-05~~ | ~~Prompt del moderador~~ | — | ✅ | **Cerrado por ADR-012** | — |
| E-06 | Los schemas de salida | [13](../03-capacidades-de-ia/03-rubricas-y-prompts.md) §6-9 | 📝 | Cada dueño de módulo | Con su función |
...
> está en [04](../03-capacidades-de-ia/02-funciones-de-ia.md), Parte 3: un docente puntúa las cinco dimensiones y revisa que
...
| # | Qué | Dónde | Marca | Quién lo define | Cuándo |
|---|---|---|---|---|---|
| E-07 | La transcripción de ejemplo (#17) | [04](../03-capacidades-de-ia/02-funciones-de-ia.md) Parte 3 | 🧪 | Ilustra el formato. Las reales las produce P4 | Paso 6 |
| E-08 | **Los puntajes de esa transcripción** | [04](../03-capacidades-de-ia/02-funciones-de-ia.md) Parte 3 | 🧪 | 🔴 **Inventados por nosotros.** Los reales los ponen personas | Paso 6 |
| E-09 | Tamaño del conjunto: 40 (o 10 reducido) | [04](../03-capacidades-de-ia/02-funciones-de-ia.md) Parte 3 | 📝 | PO | 🔴 Esta semana |
| E-10 | La distribución de perfiles (8 pasivos, 6 jailbreak…) | [04](../03-capacidades-de-ia/02-funciones-de-ia.md) Parte 3 | 📝 | P4 + docentes | Paso 6 |
...
| # | Qué | Dónde | Marca | Quién lo define | Cuándo |
|---|---|---|---|---|---|
| E-11 | Los campos entre `[corchetes]` | [07](../04-seguridad-datos-y-cumplimiento/02-datos-retencion-y-terminos.md) Parte B | ⬜ | PO + institución | Antes del release |
| E-12 | **Anexo A: los proveedores en uso** | [07](../04-seguridad-datos-y-cumplimiento/02-datos-retencion-y-terminos.md) Parte B | ⬜ | Nosotros — **solo nosotros lo sabemos** | Después del paso 7 |
| E-13 | El porcentaje del modificador de XP | [07](../04-seguridad-datos-y-cumplimiento/02-datos-retencion-y-terminos.md) Parte B §4.2 | ⬜ | ADMIN, es PAR-05 | Configuración |
| E-14 | El texto completo | [07](../04-seguridad-datos-y-cumplimiento/02-datos-retencion-y-terminos.md) Parte B | 📝 | 🔴 **Revisión jurídica** | Antes del release |

### Esquema de datos

| # | Qué | Dónde | Marca | Quién lo define | Cuándo |
|---|---|---|---|---|---|
| E-15 | Nombres de campos y tablas | [11](03-glosario-y-metadata.md) Parte B | 📝 | P5 | 🔴 Paso 2 — esta semana |
| E-16 | **Qué es un "mensaje trivial"** (¿menos de cuántos caracteres?) | [11](03-glosario-y-metadata.md) Parte B | ⬜ | P3, ajustando contra el golden set | Paso 4 |
| E-17 | Las 8 colisiones del glosario | [11](03-glosario-y-metadata.md) Parte A | 📝 | 🔴 **La sesión de integración**, no nosotros solos | Esta semana |
| E-18 | Campos del contrato de eventos | [02](../02-arquitectura-y-plataforma/01-arquitectura-y-stack.md) Parte 3 | 📝 | 🔴 **Tema 11**, antes de que lo cierren | Esta semana |

### Números y umbrales

| # | Qué | Dónde | Marca | Cómo se define de verdad |
|---|---|---|---|---|
| E-19 | **Modelo por función** | [03](../03-capacidades-de-ia/01-modelos-costos-y-contexto.md) §1 | 📝 | 🔴 **La calibración decide**, no la recomendación. Paso 7 |
| E-20 | 10 mensajes de tutor por desafío | [03](../03-capacidades-de-ia/01-modelos-costos-y-contexto.md) §4 | 🔢 | Medir la adopción real en el piloto |
| E-21 | 50% de desafíos prácticos | [03](../03-capacidades-de-ia/01-modelos-costos-y-contexto.md) §4 | 🔢 | Depende del roadmap que arme el profesor |
| E-22 | Umbrales de RF-IA-22 (15 / 60 / 20) | [08](02-decisiones-y-pendientes.md) D-8 | 📝 | ADMIN, con datos de uso real |
| E-23 | 3 chunks en el retrieval | [03](../03-capacidades-de-ia/01-modelos-costos-y-contexto.md) §6 | 📝 | Las 30 preguntas etiquetadas: comparar 3 vs 5 vs 8 |
| E-24 | Piso de similitud del retrieval | [05](../04-seguridad-datos-y-cumplimiento/01-seguridad-y-guardarrailes.md) | ⬜ | Mirando la tasa de "no encontrado" |
| E-25 | Tamaño de chunk 500-800 tokens | [04](../03-capacidades-de-ia/02-funciones-de-ia.md) Parte 1 | 📝 | Probar con el material real |
| E-26 | 20 min de drenado en el pico | [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md) Parte 2 | 🔢 | La prueba de carga del DoD 10 |
...
| E-28 | Los 7 estados del componente del tutor | [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md) Parte 4 | 📝 | P6 |
| E-29 | Los mensajes de error al usuario | [06](../06-operacion-calidad-y-pruebas/01-operacion-e-ingenieria.md) Parte 4 | 📝 | P6 + PO |
